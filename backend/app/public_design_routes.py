...
from .routes.app_settings import public_settings
from .routes.products import VISIBLE_STATUSES, product_available_quantity

router = APIRouter(tags=["V71 Javni dizajn"])
SUPPORT_EMAIL = "support@klikzarada.rs"


def _public_brand() -> dict:
    cfg = public_settings() or {}
    brand_name = (cfg.get("brand_name") or "KlikZarada").strip()
    tagline = (cfg.get("brand_tagline") or "Mikro zadaci · merljivi rezultati").strip()
    motto = (cfg.get("brand_motto") or "Pametne kampanje, jasne isplate i kontrolisani tokovi.").strip()
    finance = cfg.get("finance_accounts") or {}
    initials = "".join(part[0] for part in brand_name.split() if part)[:2].upper() or "KZ"
    return {
        "name": brand_name,
        "tagline": tagline,
        "motto": motto,
...
"""
CSS += """
/* V111: jasnija pocetna ikonica za restorane i funkcionalan profil demo. */
.restaurant-icon-v111{width:58px;height:58px;border-radius:18px;background:linear-gradient(145deg,#fff7de,#eaf8ef);border:1px solid #d8eadf;display:grid;place-items:center;box-shadow:0 10px 24px rgba(15,61,46,.1);font-size:34px}.profile-status-v111{margin-top:14px;background:#eef8f1;border:1px solid #cfe3d6;border-radius:16px;padding:12px 14px;color:var(--green);font-weight:900}.profile-form-v111{display:grid;gap:12px}.profile-form-v111 label{font-weight:900;color:var(--green)}.profile-form-v111 input{width:100%;margin-top:7px}
"""
CSS += """
.impact-panel{background:linear-gradient(135deg,#0f3d2e,#175f45);border:1px solid rgba(255,255,255,.18);border-radius:28px;color:#fff;padding:24px;box-shadow:var(--shadow);overflow:hidden;position:relative}.impact-panel:after{content:"";position:absolute;width:240px;height:240px;border-radius:50%;right:-80px;top:-90px;background:rgba(174,232,201,.18)}.impact-panel>*{position:relative;z-index:1}.impact-panel h2,.impact-panel h3{color:#fff}.impact-panel .lead{color:#edfdf5}.impact-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:18px 0}.impact-stat{background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.16);border-radius:18px;padding:16px;min-height:118px}.impact-stat b{display:block;font-size:26px;line-height:1;color:#fff}.impact-stat span{display:block;margin-top:8px;color:#dff6ec;font-weight:850;font-size:13px;line-height:1.35}.article-layout{display:grid;grid-template-columns:minmax(0,1fr) 320px;gap:18px;align-items:start}.article-card{background:rgba(255,255,255,.9);border:1px solid var(--line);border-radius:26px;padding:24px;box-shadow:var(--shadow-soft)}.article-card h2{margin-top:26px}.article-card h2:first-child{margin-top:0}.article-card h3{margin-top:20px}.article-card li{margin:9px 0;line-height:1.55}.source-list{display:grid;gap:10px}.source-list a,.source-list span{display:block;background:#fffdf8;border:1px solid var(--line);border-radius:14px;padding:12px;font-weight:850;color:var(--green)}.partner-impact{margin:18px 0;background:#f4fbf7;border:1px solid #dceee4;border-radius:20px;padding:16px}@media(max-width:900px){.impact-grid,.article-layout{grid-template-columns:1fr 1fr}.article-layout{grid-template-columns:1fr}}@media(max-width:640px){.impact-grid{grid-template-columns:1fr}.impact-panel{padding:18px}.article-card{padding:18px}}
"""

CSS += """
/* V109: javni sajt ide preko cele sirine, bez velikog praznog prostora desno. */
:root{--public-page-w:calc(100% - 48px)}.wrap,.nav-inner,.app-footer{width:var(--public-page-w);max-width:none;margin-left:auto;margin-right:auto}.wrap{padding:24px 0}.nav-inner{padding-left:0;padding-right:0}.hero,.section,.layout,.article-layout,.gps-panel{width:100%;max-width:none}.hero{grid-template-columns:minmax(0,1.05fr) minmax(380px,.95fr);align-items:stretch}.hero-card,.visual,.detail,.card,.article-card{min-width:0}.visual{width:100%;justify-self:stretch;display:flex;flex-direction:column;justify-content:flex-end}.grid.cards{grid-template-columns:repeat(3,minmax(0,1fr))}.grid.two{grid-template-columns:repeat(2,minmax(0,1fr))}.grid.four,.category-grid,.impact-grid{grid-template-columns:repeat(4,minmax(0,1fr))}.layout{grid-template-columns:minmax(260px,320px) minmax(0,1fr)}.app-footer{padding-left:0;padding-right:0}.motto-panel{width:100%;max-width:none}.motto-grid{grid-template-columns:repeat(4,minmax(0,1fr))}body{overflow-x:hidden}@media(max-width:1100px){.hero,.gps-panel,.article-layout{grid-template-columns:1fr}.layout{grid-template-columns:1fr}.visual{min-height:320px}.grid.cards,.grid.two,.grid.four,.category-grid,.impact-grid,.motto-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:700px){:root{--public-page-w:calc(100% - 24px)}.wrap{padding:16px 0}.nav-inner{align-items:flex-start;flex-direction:column}.hero-card{padding:24px}h1{font-size:42px}.grid.cards,.grid.two,.grid.four,.category-grid,.impact-grid,.motto-grid{grid-template-columns:1fr}.searchbar{flex-direction:column}.searchbar input{width:100%}}
"""

CSS += """
.nav-links{flex:1;justify-content:center}.nav-actions{display:flex;align-items:center;gap:10px;flex:0 0 auto;margin-left:auto;flex-wrap:wrap}.nav-actions .btn{min-height:44px;padding:10px 16px;border-radius:999px;white-space:nowrap}@media(max-width:700px){.nav-actions{width:100%;justify-content:flex-start}.nav-actions .btn{flex:1 1 132px}}
"""

def logo():
    brand = _public_brand()
    return f"""<a class='brand' href='/pocetna' aria-label='KlikZarada početna'><span class='brand-badge'>{brand['initials']}</span><div><div class='brand-title'>{brand['name']}</div><div class='brand-sub'>{brand['tagline']}</div></div></a>"""

def nav(active="pocetna"):
    items=[("pocetna","/pocetna","Početna"),("zadaci","/zadaci","Zadaci"),("korisnici","/profil","Za korisnike"),("oglasivaci","/partner/onboarding","Za oglašivače"),("cenovnik","/cenovnik","Cenovnik"),("blog","/blog","Blog"),("kontakt","/kontakt","Kontakt")]
    links="".join([f"<a class='{ 'active' if k==active else '' }' href='{href}'>{label}</a>" for k,href,label in items])
    return f"<div class='nav'><div class='nav-inner'>{logo()}<div class='nav-links'>{links}</div><div class='nav-actions'><a class='btn secondary' href='/profil'>Moj panel</a><button class='btn secondary' type='button' onclick=\"fetch('/auth/admin/logout',{{method:'POST'}}).finally(()=>location.href='/admin-login')\">Odjava</button></div></div></div>"

def footer():
    brand = _public_brand()
    finance = brand["finance"]
    advertiser_line = finance.get("advertiser_deposit_recipient") or "KlikZarada budžet"
    payout_line = finance.get("seller_payout_recipient") or "KlikZarada isplata"
    return f"""<footer class='app-footer'><section class='motto-panel'><div class='motto-brand'><span class='brand-badge'>{brand['initials']}</span><div><div class='motto-title'>{brand['name']}</div><div class='motto-sub'>{brand['tagline']} · <a href='mailto:{SUPPORT_EMAIL}'>{SUPPORT_EMAIL}</a></div></div></div><div class='motto-grid'><div class='motto-item'><div class='motto-icon'>🛡️</div>Proverene kampanje i zadaci<small>Oglašivači, korisnici i admin rade kroz jasan tok odobravanja.</small></div><div class='motto-item'><div class='motto-icon'>🔒</div>Sigurne prijave i isplate<small>Uplata na oglašivački račun i isplata korisnika iz kontrole u adminu.</small></div><div class='motto-item'><div class='motto-icon'>🤝</div>Zajednica koja radi zajedno<small>Korisnici, oglašivači i admin grade rezultate.</small></div><div class='motto-item'><div class='motto-icon'>📈</div>Za rast bez haosa<small>Manje ručnog rada, više automatizacije.</small></div></div><div class='motto-grid' style='margin-top:14px'><div class='motto-item'><div class='motto-icon'>💳</div>Uplata oglašivača<small>{advertiser_line}</small></div><div class='motto-item'><div class='motto-icon'>🏦</div>Isplata korisnika<small>{payout_line}</small></div><div class='motto-item'><div class='motto-icon'>⚙️</div>Sistem pod kontrolom<small>{brand['motto']}</small></div><div class='motto-item'><div class='motto-icon'>🧾</div>Računi i evidencija<small>Fakture, provizija i istorija transakcija u jednom mestu.</small></div></div></section></footer>"""

def page(title, body, active="pocetna"):
    brand = _public_brand()
    pwa_head = """<link rel='manifest' href='/admin-assets/manifest.webmanifest'><meta name='theme-color' content='#0b5cd6'><meta name='apple-mobile-web-app-capable' content='yes'><meta name='apple-mobile-web-app-title' content='KlikZarada'><link rel='apple-touch-icon' href='/admin-assets/icons/icon-192.png'>"""
    ...
    html=f"""<!doctype html><html lang='sr'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1'><title>{title} — {brand['name']}</title>{pwa_head}<style>{CSS}</style></head><body>{nav(active)}<main class='wrap'>{body}</main>{footer()}{pwa_script}</body></html>"""
    return HTMLResponse(html, headers={"Cache-Control":"no-store, max-age=0"})

def offer_card(title="Istaknuta kampanja", seller="Aktivan oglašivač", price="360 RSD", old="600 RSD", badge="-40%", product_id=None, pickup="24h", address="Beograd"):
...
def product_offer_card(db: Session, product: models.Product):
    ...
    store = product.store.name if product.store else "KlikZarada partner"
...
function aiProductHtml(product){
  ...
  var store = product.store_name || 'KlikZarada partner';
...
def home_page():
    brand = _public_brand()
    finance = brand["finance"]
    deposit_account = finance.get("advertiser_deposit_account") or "Račun za uplatu se podešava u adminu"
    deposit_recipient = finance.get("advertiser_deposit_recipient") or "KlikZarada budžet"
    payout_account = finance.get("seller_payout_account") or "Račun za isplatu se podešava u adminu"
    payout_recipient = finance.get("seller_payout_recipient") or "KlikZarada isplata"
    body=f"""
    <section class='hero'>
      <div class='hero-card'>
        <span class='badge'>⚡ Brzi start bez demo podataka</span>
        <h1>Pametne kampanje.<br>Jasne isplate.<br><span>Jedna kontrolna tabla.</span></h1>
        <p class='lead'>Kreiraj prvi oglas, prvog korisnika i prvi zadatak iz admina. Uplate oglašivača i isplate korisnika vode se kroz račune koje vidiš odmah, bez kopanja po kodu.</p>
        <div class='gps-actions'>
          <a class='btn' href='/za-oglasivace'>Za oglašivače</a>
          <a class='btn secondary' href='/cenovnik'>Cenovnik</a>
          <a class='btn secondary' href='/kontakt'>Kontakt</a>
        </div>
        <div class='stats'>
          <div class='stat'><b>1</b><span>prvi oglas</span></div>
          <div class='stat'><b>1</b><span>prvi korisnik</span></div>
          <div class='stat'><b>1</b><span>prvi zadatak</span></div>
          <div class='stat'><b>24h</b><span>cena banera po danu</span></div>
        </div>
      </div>
      <div class='visual'>
        <div class='badge' style='width:max-content'>Budžet i isplate</div>
        <h2>Računovodstvo bez haosa</h2>
        <div class='grid two'>
          <div class='stat'><b>Uplata</b><span>{deposit_recipient}</span></div>
          <div class='stat'><b>Isplata</b><span>{payout_recipient}</span></div>
          <div class='stat'><b>{brand['tagline']}</b><span>{brand['motto']}</span></div>
          <div class='stat'><b>25%</b><span>platformska provizija</span></div>
        </div>
        <p class='muted'>Računi se menjaju u adminu i odmah se pojavljuju na javnoj strani.</p>
      </div>
    </section>
    <section class='section'>
      <div class='section-head'><div><h2>Računi i tok novca</h2><p class='lead'>Javno mesto za uplatu budžeta oglašivača i pregled računa za isplatu korisnicima.</p></div><a class='chip' href='/cenovnik'>Pogledaj cenovnik</a></div>
      <div class='grid two'>
        <div class='detail'>
          <span class='badge'>Račun za uplatu oglašivača</span>
          <h2 style='margin-top:12px'>Budžet kampanje</h2>
          <p class='lead'>Račun: <b>{deposit_account}</b><br>Primalac: <b>{deposit_recipient}</b></p>
          <div class='gps-actions'><a class='btn' href='/za-oglasivace'>Kreiraj kampanju</a><a class='btn secondary' href='/cenovnik'>Pogledaj cenu</a></div>
        </div>
        <div class='detail'>
          <span class='badge'>Račun za isplatu korisnika</span>
          <h2 style='margin-top:12px'>Isplate i nagrade</h2>
          <p class='lead'>Račun: <b>{payout_account}</b><br>Primalac: <b>{payout_recipient}</b></p>
          <div class='gps-actions'><a class='btn' href='/kontakt'>Pošalji upit</a><a class='btn secondary' href='/blog'>Kako radi</a></div>
        </div>
      </div>
    </section>
    <section class='section'>
      <div class='section-head'><div><h2>Kako radi admin flow</h2><p class='lead'>Bez demo toka i bez ručnog prepisivanja. Sve ide preko admin panela.</p></div><span class='chip'>Prva podešavanja u adminu</span></div>
      <div class='grid four'>
        <div class='card'><div class='card-body'><h3>1. Uplata</h3><p>Oglašivač uplaćuje budžet na navedeni račun i admin vidi stanje u finansijama.</p></div></div>
        <div class='card'><div class='card-body'><h3>2. Kreiranje</h3><p>Admin pravi prvog oglašivača, korisnika i prvi zadatak bez demo podataka.</p></div></div>
        <div class='card'><div class='card-body'><h3>3. Provera</h3><p>Anti-fraud, ručna potvrda i evidencija dokaza stoje u adminu pre svakog puštanja live.</p></div></div>
        <div class='card'><div class='card-body'><h3>4. Isplata</h3><p>Po završetku rada korisnici dobijaju isplatu sa računa koji je unapred definisan u sistemu.</p></div></div>
      </div>
    </section>
    <section class='section impact-panel'>
      <span class='badge'>Zašto postoji KlikZarada</span>
      <h2>Manje ručnog rada, više automatizacije i jasna monetizacija.</h2>
      <p class='lead'>Platforma povezuje oglašivače, korisnike i admin tok tako da se vidi šta je plaćeno, šta je odrađeno i šta je spremno za isplatu.</p>
      <div class='impact-grid'>
        <div class='impact-stat'><b>24h</b><span>baneri i istaknute pozicije se obračunavaju po danu.</span></div>
        <div class='impact-stat'><b>1 klik</b><span>do podešavanja brenda, slogana i računa u adminu.</span></div>
        <div class='impact-stat'><b>0 demo</b><span>javni sajt i admin treba da izgledaju kao prava live platforma.</span></div>
        <div class='impact-stat'><b>100%</b><span>računa i isplata vidi se kroz istu kontrolnu tablu.</span></div>
      </div>
    </section>
    """
...
@router.get("/cenovnik", response_class=HTMLResponse)
def pricing_page():
    brand = _public_brand()
    finance = brand["finance"]
    deposit_account = finance.get("advertiser_deposit_account") or "Račun za uplatu se podešava u adminu"
    ...
    <section class='hero-mini'>
      <div class='detail'>
        <span class='badge'>Cenovnik po danu</span>
        <h1 style='font-size:42px'>Sve banner pozicije obračunate su na 24h nivou.</h1>
        <p class='lead'>Oglašivač odmah vidi cenu po danu, a admin lako upravlja budžetom, finansijama i rasporedom banera.</p>
        <div class='gps-actions'><a class='btn' href='/za-oglasivace'>Kreiraj kampanju</a><a class='btn secondary' href='/kontakt'>Kontakt prodaje</a></div>
      </div>
      <aside class='detail'>
        <h2>Računi</h2>
        <p class='muted'><b>Uplata oglašivača:</b><br>{deposit_account}</p>
        <p class='muted'><b>Isplata korisnika:</b><br>{payout_account}</p>
        <p class='muted'>Sve izmene unosiš u adminu i odmah postaju vidljive na javnoj strani.</p>
      </aside>
    </section>
    <section class='section'>
      <div class='grid cards'>
        <article class='card'><div class='card-body'><span class='badge'>Home top</span><h2>9.500 RSD / 24h</h2><p class='lead'>Veliki baner na početnoj za glavnu poruku i premium poziciju.</p></div></article>
        <article class='card'><div class='card-body'><span class='badge'>Home mid</span><h2>5.500 RSD / 24h</h2><p class='lead'>Srednji baner na početnoj za dodatnu vidljivost i stabilan lead flow.</p></div></article>
        <article class='card'><div class='card-body'><span class='badge'>Featured</span><h2>1.800 RSD / 24h</h2><p class='lead'>Istaknuta kampanja za brže izdvajanje ponude u listama i pretragama.</p></div></article>
      </div>
    </section>
    <section class='section'>
      <div class='grid cards'>
        <article class='card'><div class='card-body'><span class='badge'>Highlighted</span><h2>1.200 RSD / 24h</h2><p class='lead'>Lagano isticanje bez agresivnog vizuala.</p></div></article>
        <article class='card'><div class='card-body'><span class='badge'>Platform fee</span><h2>25%</h2><p class='lead'>Provizija se vidi u finansijama i koristi za platformu i podršku.</p></div></article>
        <article class='card'><div class='card-body'><span class='badge'>Isplate</span><h2>Kontrolisano</h2><p class='lead'>Korisnici se isplaćuju sa definisanog računa nakon potvrde zadatka.</p></div></article>
      </div>
    </section>
    """
    return page("Cenovnik", body, "cenovnik")


@router.get("/za-oglasivace", response_class=HTMLResponse)
def advertiser_alias_page():
    return partner_onboarding_page()
...
def blog_alias_page():
    ...
    <section class='article-layout'>
      <article class='article-card'>
        ...
        <h1 style='font-size:46px'>Kako KlikZarada obrađuje kampanje, račune i isplate</h1>
        <p class='lead'>Javni sajt pokazuje ponude i cenovnik po danu, dok admin drži brand, račune i finansije pod jednom kontrolom.</p>
        <h2>1. Uplata budžeta</h2>
        <p>Oglašivač uplaćuje budžet na definisani račun i admin to vidi u finansijskoj konzoli.</p>
        <h2>2. Objavljivanje kampanje</h2>
        <p>Baneri, istaknute kampanje i zadaci se objavljuju sa jasnom cenom po danu i bez ručnog prepisivanja.</p>
        <h2>3. Isplate i evidencija</h2>
        <p>Kada je zadatak odrađen i potvrđen, korisnik ulazi u isplatu sa računa koji je definisan u adminu.</p>
        <div class='gps-actions'><a class='btn' href='/cenovnik'>Pogledaj cenovnik</a><a class='btn secondary' href='/za-oglasivace'>Za oglašivače</a></div>
      </article>
      <aside class='article-card'>
        <h2>Brze veze</h2>
        <div class='source-list'><a href='/cenovnik'>Cenovnik</a><a href='/admin/finance-console'>Finansije</a><a href='/podrska'>Kontakt i podrška</a></div>
      </aside>
    </section>
    """
...
def contact_alias_page():
    return support_page()

@router.get("/problem-bacanja-hrane", response_class=HTMLResponse)
@router.get("/zasto-sacuvaj-hranu", response_class=HTMLResponse)
def food_waste_problem_page():
    body=f"""<section class='article-layout'><article class='article-card'><span class='badge'>Problem koji rešavamo</span><h1 style='font-size:46px'>Bacanje hrane u Srbiji</h1><p class='lead'>Dok se deo građana bori da obezbedi osnovne namirnice, velika količina bezbedne i jestive hrane svakodnevno završava kao otpad. KlikZarada postoji da višak hrane dobije drugu šansu: kroz brzu ponudu, rezervaciju i preuzimanje u lokalnoj zajednici.</p><div class='impact-grid'><div class='impact-stat' style='background:#f4fbf7;color:var(--green)'><b style='color:var(--green)'>2.000 t</b><span style='color:var(--muted)'>procena dnevnog otpada od hrane u domaćim izveštajima.</span></div><div class='impact-stat' style='background:#f4fbf7;color:var(--green)'><b style='color:var(--green)'>730k-780k t</b><span style='color:var(--muted)'>procena godišnje količine bačene hrane.</span></div><div class='impact-stat' style='background:#f4fbf7;color:var(--green)'><b style='color:var(--green)'>247k t</b><span style='color:var(--muted)'>procena jestive hrane koja propadne godišnje.</span></div><div class='impact-stat' style='background:#f4fbf7;color:var(--green)'><b style='color:var(--green)'>240-350M €</b><span style='color:var(--muted)'>procena ekonomskog gubitka godišnje.</span></div></div><h2>Ko najviše baca hranu?</h2><p>Istraživanja pokazuju da odgovornost nastaje kroz ceo lanac, od proizvodnje i prerade, preko ugostiteljstva i trgovine, do domaćinstava. Zato rešenje mora biti praktično: hrana koja je bezbedna, ali neće biti prodata na vreme, treba da što brže dođe do kupca.</p><ul><li><b>Domaćinstva:</b> hrana se često baci jer se pokvari, kupi se previše ili se zaboravi u frižideru.</li><li><b>Proizvodnja i prerada:</b> gubici nastaju kroz transport, skladištenje i neefikasne tokove robe.</li><li><b>Ugostiteljstvo:</b> restorani, hoteli i brza hrana imaju viškove zbog porcija, pripreme unapred i promenljive potražnje.</li><li><b>Maloprodaja:</b> prodavnice povlače robu zbog roka, oštećenja ambalaže ili estetskih standarda.</li></ul><h2>Zašto su pekare, restorani i prodavnice ključni?</h2><h3>Pekare</h3><p>Pekare moraju da imaju pune vitrine do kraja dana, a hleb i peciva sutradan često više nemaju tržišnu vrednost. Ako se višak objavi na vreme, kupac može da ga rezerviše po nižoj ceni, a lokal smanjuje bacanje i vraća deo vrednosti.</p><h3>Restorani</h3><p>Kod restorana je problem najčešće u prekomernoj pripremi, promenljivoj potražnji i vremenski osetljivim obrocima. Kratak prozor za preuzimanje i jasna rezervacija rešavaju veliki deo tog problema.</p><h3>Prodavnice</h3><p>Marketima i lokalnim prodavnicama ostaju proizvodi pred istek roka, voće i povrće nepravilnog izgleda ili paketi koji više nisu idealni za punu cenu. Takva hrana može biti potpuno upotrebljiva ako se kupcu ponudi jasno, brzo i odgovorno.</p><h2>Zašto se hrana ne donira uvek?</h2><p>Humanitarne organizacije i privreda često ukazuju na prepreke kao što su PDV na donacije hrane, strah od kazni i nejasna administracija. Donacije su važne, ali nisu jedini kanal. Komercijalno spasavanje hrane kroz popust može da radi svakog dana, bez čekanja velikih sistemskih promena.</p><h2>Ekološki efekat</h2><p>Kada hrana završi na deponiji, truljenjem oslobađa metan, gas sa snažnim efektom staklene bašte. Manje otpada znači manje emisija, manje troškova i bolji odnos prema hrani koju smo već proizveli, transportovali i platili.</p><h2>Kako KlikZarada pomaže?</h2><ul><li>Partner objavi višak hrane uz jasnu cenu, količinu i vreme preuzimanja.</li><li>Kupac pronalazi obrok kroz AI pomoćnika, mapu ili kategorije.</li><li>Rezervacija se potvrđuje digitalnom kartom i QR kodom.</li><li>Partner smanjuje otpad, kupac štedi novac, a zajednica čuva resurse.</li></ul><div class='gps-actions'><a class='btn' href='/ponude'>Pronađi obrok</a><a class='btn secondary' href='/partner/onboarding'>Uključi lokal</a></div></article><aside class='article-card'><h2>Izvori i napomena</h2><p class='muted'>Brojke su informativne i oslanjaju se na javno dostupne procene domaćih i međunarodnih organizacija. Pre većih PR kampanja treba dodati finalne pravne i analitičke reference.</p><div class='source-list'><a href='https://www.unep.org/resources/publication/food-waste-index-report-2024' target='_blank' rel='noopener'>UNEP Food Waste Index Report 2024</a><span>Centar za unapređenje životne sredine (CUŽS)</span><span>FAO i NALED analize o otpadu hrane i donacijama</span><span>Banka hrane Vojvodina / RTS javni navodi</span></div><div class='partner-impact'><h3>Za partnere</h3><p>Ova stranica je korisna u prodaji: pokazuje da aplikacija nije samo popust, nego alat za manji gubitak, bolji imidž i odgovornije poslovanje.</p><a class='chip' href='mailto:{SUPPORT_EMAIL}'>Kontakt: {SUPPORT_EMAIL}</a></div></aside></section>"""
    return page("Bacanje hrane u Srbiji", body, "zasto")

@router.get("/ponude", response_class=HTMLResponse)
@router.get("/zadaci", response_class=HTMLResponse)
def listing_page(category: str | None = Query(default=None), db: Session = Depends(get_db)):
    ...
    return page("Zadaci", body, "zadaci")
...
def reserve_product_page(product_id: int, db: Session = Depends(get_db)):
    ...
    body=f"""<section class='hero-mini'><div class='detail'><span class='badge'>Rezervacija ponude</span><h1 style='font-size:42px'>{product.name}</h1><p class='lead'>{store}<br>{address}</p><div class='grid two'><div class='stat'><b>{price} RSD</b><span>cena za preuzimanje</span></div><div class='stat'><b>{old} RSD</b><span>redovna cena</span></div><div class='stat'><b>{available if available is not None else 'Dostupno'}</b><span>preostala količina</span></div><div class='stat'><b>{product.pickup_window or 'Danas'}</b><span>vreme preuzimanja</span></div></div></div><aside class='detail'><h2>Podaci kupca</h2><div class='grid'><input class='field' id='customerName' value='KlikZarada kupac' placeholder='Ime i prezime'><input class='field' id='customerPhone' value='+38160111000' placeholder='Telefon'><input class='field' id='customerEmail' value='kupac@klikzarada.rs' placeholder='Email'><input class='field' id='quantity' type='number' min='1' value='1'><button class='btn' type='button' onclick='reserveNow()'>Potvrdi rezervaciju</button><button class='btn secondary' id='pickupBtn' type='button' onclick='payOnPickup()' disabled>Plati pri preuzimanju</button><button class='btn secondary' id='demoPayBtn' type='button' onclick='demoPay()' disabled>Online plaćanje</button><div id='reserveStatus' class='map-status'>Rezervacija još nije poslata.</div></div></aside></section><script>
...
async function demoPay(){{
  if(!reservationCode) return;
  const phone = document.getElementById('customerPhone').value;
  const r = await fetch('/payments/reservations/'+reservationCode+'/pay', {{method:'POST', headers:{{'Content-Type':'application/json'}}, body:JSON.stringify({{customer_phone:phone,payment_method:'online_card'}})}});
  const data = await r.json();
...
def partner_onboarding_page():
    body="""<section class='hero-mini'><div class='detail'><span class='badge'>Za oglašivače</span><h1 style='font-size:42px'>Uključi lokal i prvu kampanju za nekoliko minuta</h1><p class='lead'>Partner odmah dobija ID, PIN i prvu javnu kampanju. Kasnije admin može da uradi dodatnu proveru i finansijski ugovor.</p><div class='partner-impact'><h3>Zašto je ovo dobro za lokal?</h3><p>Oglašivač dobija jasan prostor za brend i kampanje, bez ručnog prepisivanja i bez demo osećaja. Sve je spremno za uplatu, objavu i kontrolu.</p><a class='chip' href='/cenovnik'>Vidi cenovnik</a></div><div class='grid two'><div class='stat'><b>1</b><span>prijava lokala</span></div><div class='stat'><b>PIN</b><span>ulaz u partner panel</span></div><div class='stat'><b>QR</b><span>potvrda preuzimanja</span></div><div class='stat'><b>25%</b><span>platformska provizija</span></div></div></div><aside class='detail'><h2>Podaci partnera</h2><div class='grid'><input class='field' id='businessName' value='KlikZarada lokal' placeholder='Naziv lokala'><select class='field' id='category'><option value='restoran'>Restoran</option><option value='pekara'>Pekara</option><option value='market'>Market</option><option value='voće i povrće'>Voće i povrće</option></select><input class='field' id='city' value='Beograd' placeholder='Grad'><input class='field' id='address' value='Knez Mihailova 10' placeholder='Adresa'><input class='field' id='contactName' value='Partner Kontakt' placeholder='Kontakt osoba'><input class='field' id='phone' value='+38160111666' placeholder='Telefon'><input class='field' id='email' value='partner@klikzarada.rs' placeholder='Email'><input class='field' id='offerName' value='Prva kampanja' placeholder='Naziv prve kampanje'><input class='field' id='originalPrice' type='number' value='600' placeholder='Stara cena'><input class='field' id='discountedPrice' type='number' value='360' placeholder='Nova cena'><input class='field' id='quantity' type='number' value='5' placeholder='Količina'><input class='field' id='pickupWindow' value='18:00 - 19:00' placeholder='Vreme preuzimanja'><button class='btn secondary' type='button' onclick='usePartnerGps()'>Uzmi GPS</button><button class='btn' type='button' onclick='onboardPartner()'>Uključi partnera</button><div id='onboardStatus' class='map-status'>Spremno za unos.</div></div></aside></section><script>
...
def partner_live_panel_page(store_id: int | None = Query(default=None), pin: str | None = Query(default=None), db: Session = Depends(get_db)):
    ...
    body=f"""<div class='layout'><aside class='sidebar'><h3 style='color:white'>{store.name}</h3><a class='side-link active'>Pregled</a><a class='side-link' href='/partner/preuzimanje'>Preuzimanje</a><a class='side-link' href='/ponude'>Javne ponude</a><a class='side-link' href='/partner/onboarding'>Novi partner</a><p style='color:#d9f4e8'>ID: {store.id}<br>PIN: {store.seller_pin}</p></aside><section><div class='section-head'><div><h1 style='font-size:42px'>Partner panel</h1><p class='lead'>Upravljaj ponudama i prati rezervacije u kontrolisanom režimu.</p></div><a class='btn' href='/partner/preuzimanje'>Potvrdi preuzimanje</a></div><div class='grid four'><div class='kpi'><b>{len(products)}</b>Ponude</div><div class='kpi'><b>{len(reservations)}</b>Rezervacije</div><div class='kpi'><b>{sum(1 for r in reservations if r.status=='picked_up')}</b>Preuzeto</div><div class='kpi'><b>{int(sum(r.platform_fee_amount or 0 for r in reservations if r.seller_payout_status=='commission_due'))}</b>RSD provizija</div></div><section class='section detail'><h2>Dodaj novu ponudu</h2><div class='grid two'><input class='field' id='name' value='Nova ponuda' placeholder='Naziv'><input class='field' id='cat' value='restoran' placeholder='Kategorija'><input class='field' id='old' type='number' value='600'><input class='field' id='price' type='number' value='360'><input class='field' id='qty' type='number' value='5'><input class='field' id='pickup' value='18:00 - 19:00'><button class='btn' onclick='addOffer()'>Objavi ponudu</button><div id='offerStatus' class='map-status'>Spremno.</div></div></section><section class='section'><div class='section-head'><h2>Ponude</h2></div><div class='grid two' id='products'>{product_rows}</div></section><section class='section'><div class='section-head'><h2>Rezervacije</h2></div><div class='grid two'>{reservation_rows}</div></section></section></div><script>
...
def profile_page():
    return page("Profil", """<div class='layout'><aside class='sidebar'><div class='avatar'>IT</div><h3 style='color:white'>Moj profil</h3><a class='side-link active' href='/profil'>Lični podaci</a><a class='side-link' href='/moje-rezervacije'>Rezervacije</a><a class='side-link' href='/podrska'>Obaveštenja i podrška</a></aside><section><h1 style='font-size:42px'>Profil korisnika</h1><div class='grid two'><div class='detail'><h2>Lični podaci</h2><form id='profileFormV111' class='profile-form-v111'><label>Ime<input class='field' name='name' value='KlikZarada korisnik'></label><label>Email<input class='field' name='email' type='email' value='korisnik@example.com'></label><button class='btn' type='submit'>Sačuvaj izmene</button><div id='profileStatusV111' class='profile-status-v111'>Spremno za izmene profila.</div></form></div><div class='detail'><h2>Tvoj uticaj</h2><div class='stats'><div class='stat'><b>12</b>spašenih obroka</div><div class='stat'><b>2.640 RSD</b>uštede</div></div><p class='muted'>Za pun kupac nalog otvori stranicu Moje rezervacije i potvrdi telefon.</p><a class='btn secondary' href='/moje-rezervacije'>Otvori rezervacije</a></div></div></section></div><script>document.getElementById('profileFormV111').addEventListener('submit',function(e){e.preventDefault();var data=Object.fromEntries(new FormData(e.target).entries());localStorage.setItem('sh_public_profile',JSON.stringify(data));document.getElementById('profileStatusV111').textContent='Izmene su sačuvane na ovom uređaju. Produkcioni profil ćemo vezati za potvrđen telefon kupca.';});</script>""", "rezervacije")

@router.get("/podrska", response_class=HTMLResponse)
@router.get("/support", response_class=HTMLResponse)
def support_page():
    body="""<section class='hero-mini'><div class='detail'><span class='badge'>Podrška</span><h1 style='font-size:42px'>Prijavi problem ili postavi pitanje</h1><p class='lead'>Za kampanju, plaćanje, preuzimanje, prodavca ili tehnički problem. Prijava odmah ulazi u admin support listu. Direktan kontakt: <a class='chip' href='mailto:__SUPPORT_EMAIL__'>__SUPPORT_EMAIL__</a></p><div class='trust-list'><div class='trust-row'><span class='chip'>1</span><div><b>Kampanja i QR</b><p class='muted'>Pošalji kod rezervacije ako ga imaš, da tim odmah pronađe slučaj.</p></div></div><div class='trust-row'><span class='chip'>2</span><div><b>Preuzimanje i isplata</b><p class='muted'>Ako postoji problem sa isporukom ili isplatom, odmah pošalji prijavu.</p></div></div><div class='trust-row'><span class='chip'>3</span><div><b>Partner ili finansije</b><p class='muted'>Support beleži status i prosleđuje slučaj partneru, finansijama ili adminu.</p></div></div></div></div><aside class='detail'><h2>Nova prijava</h2><form id='supportForm' class='grid support-form'><input class='field' name='name' value='KlikZarada korisnik' placeholder='Ime i prezime' required><input class='field' name='phone' value='+38160111000' placeholder='Telefon'><input class='field' name='email' value='korisnik@klikzarada.rs' placeholder='Email'><input class='field' name='reservation_code' placeholder='Kod rezervacije, opciono'><select class='field' name='topic'><option value='reservation'>Rezervacija</option><option value='payment'>Plaćanje</option><option value='pickup'>Preuzimanje</option><option value='seller'>Prodavac</option><option value='food_safety'>Bezbednost hrane</option><option value='technical'>Tehnički problem</option><option value='general'>Ostalo</option></select><textarea class='field wide-field' name='message' placeholder='Opiši šta se desilo' required>Prijava za podršku KlikZarada.</textarea><button class='btn' type='submit'>Pošalji prijavu</button><div id='supportResult' class='form-result wide-field'>Spremno.</div></form></aside></section><script>
...
def privacy_public_page():
    body=f"""<section class='detail legal-text'><span class='badge'>Podaci korisnika</span><h1 style='font-size:42px'>Privatnost</h1><p class='lead'>Jasno prikazujemo koje podatke aplikacija koristi za rezervaciju, podršku, mapu i operacije platforme.</p><h2>Podaci koje obrađujemo</h2><p>Za rezervaciju se čuvaju ime, telefon, opciono email, kod rezervacije, odabrana ponuda, količina, status plaćanja i status preuzimanja.</p><h2>Lokacija i GPS</h2><p>GPS se koristi samo kada korisnik klikne da dozvoli lokaciju. Lokacija pomera mapu na ponude u blizini i ne mora biti obavezna za korišćenje aplikacije.</p><h2>Plaćanje</h2><p>Aplikacija ne čuva podatke o kartici. U početnoj fazi je primarni model plaćanje pri preuzimanju, dok se realni payment provider uključuje tek kada bude izabran i testiran.</p><h2>Podrška i brisanje podataka</h2><p>Korisnik može preko podrške ili na <a href='mailto:{SUPPORT_EMAIL}'>{SUPPORT_EMAIL}</a> zatražiti proveru, ispravku ili brisanje podataka, u skladu sa poslovnim i zakonskim obavezama.</p><div class='gps-actions'><a class='btn' href='/podrska'>Pošalji zahtev podršci</a><a class='btn secondary' href='/uslovi-koriscenja'>Uslovi</a></div></section>"""
