...
async function loadReadiness45(){try{const r=await api45('/v45/consumer-readiness'); $45('readinessScore45').textContent=`${r.readiness_score}%`; $45('readinessMsg45').textContent=r.message;}catch(e){$45('readinessScore45').textContent='—'; $45('readinessMsg45').textContent='Baza nije proverena';}}
...
function renderOffers45(data){
  ...
  if(!data.length){empty.innerHTML=`<h3>Nema ponuda za izabrane filtere</h3><p>Probaj širi radius, drugi grad ili pozovi prodavca da se prijavi. Na javnom sajtu prikazujemo samo aktivne ponude sa jasnom cenom, rokom i slikom.</p><a class="fs45-primary" href="/partner">Prijavi prodavca</a> <button id="emptyReset45" class="fs45-ghost">Prikaži sve</button>`; $45('offersGrid45').innerHTML=''; $45('emptyReset45').onclick=resetFilters45; return;}
...
async function seedDatabase45(){toast45('Početna baza se više ne učitava sa javne strane. Koristi admin alat za import i proveru.');}
